...
//    let heading = 360.0
    ...
    let rootNodeName = "747"
    
    var originalTransform:SCNMatrix4!
	
    let pusher = Pusher(
        key: /*todo: insert Pusher App Key here*/ "",
        options: PusherClientOptions(
            authMethod: .inline(secret: /*todo: insert Pusher App Secret here*/ ""),
            host: .cluster("eu")
        )
    )
    ...
    override func viewDidLoad() {
        ...
        statusTextView.textContainerInset = UIEdgeInsets.init(top: 30.0, left: 10.0, bottom: 10.0, right: 0.0)
    }
    ...
    func connectToPusher() {
        ...
        let channel = pusher.subscribe("active_flights")
        
//        var latitude
        
        let _ = channel.bind(eventName: "new-location", callback: { (data: Any?) -> Void in
            if let listFlights = data as? [[String : AnyObject]] {
                print(listFlights)
                
                self.counter += 1
                self.status = "\(self.counter)"
                
                for flight in listFlights {

//                                    let flight = listFlights[0] //todo: UNCOMMENT if foreach breaking
                                    
                    //                let aircraft = flight["aircraft"] as! [String : AnyObject]
                    //                let operatedBy = flight["operatedBy"] as! [String : AnyObject]
                                    let flightPosition = flight["flightPosition"] as! [String : AnyObject]
                                    let location = flightPosition["location"] as! [String : AnyObject]

                    //                let flightPositions = flight["flightPosition"] as! [[String : AnyObject]] //single value, which itself contains an array value
                    //                if (flightPositions.isEmpty) {
                    //                    print("Detected flight doesn't have any known location position")
                    //                    return
                    //                }
                    //
                    //                let latestPosition = flightPositions[flightPositions.count - 1] //last in array
                    //                let location = latestPosition["location"] as! [String : AnyObject] //unwrap the Position item
                    //                self.status = "Flight location received"
                                    guard let latitude = location["latitude"] as? Double,
                                    let longitude = location["longitude"] as? Double else {
                                        fatalError()
                                    }
                    //                self.heading = bearingBetweenLocations
    ...
    func updateLocation(_ latitude : Double, _ longitude : Double) {
        ...
        if self.modelNode == nil {
            guard let url = Bundle.main.url(forResource: "747", withExtension: "usdz") else { fatalError() }
let mdlAsset = MDLAsset(url: url)
let modelScene = SCNScene(mdlAsset: mdlAsset)

            
            self.modelNode = modelScene.rootNode
//            let modelScene = SCNScene(named: "art.scnassets/747.usdz"/*Car.dae"*/)!
...
//        let scale = (1.5 * Float(distance)) / //max(Float(distance/100), 1.5 )//, 3 )
//        let scale = Float(100)//max(Float(distance/100), 1.5 )//, 3 )
        let scale = max( min( Float(1000/distance), 0.5 ), 1.5 )
//        if (distance < 300) {
